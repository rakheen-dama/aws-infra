...
# Supports multiple repos via github_repos list variable.
# If github_repos is populated, those repos are trusted. If github_repo is also
# set (non-empty), it is merged into the list. This allows a single-repo
# default via github_repo or a multi-repo setup via github_repos.
# -----------------------------------------------------------------------------

locals {
  # Combine github_repo (if non-empty) with github_repos list, deduplicating
